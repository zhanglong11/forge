<!--
创建 李建敏 20200227
修改 李建敏 20200316 设计模块-任务管理-编辑实际进度-甘特图中实际进度没有显示完整，也没有拖动条
-->
<!--/**
任务名称：2868/修改bug
开发人员：李建敏
...
<template>
  <div>
    <el-form style="margin-top: 15px;" inline>
      <el-form-item label="参考基准" label-width="70px">
      ...
      <el-form-item label="视图" class="fr" label-width="40px" style="margin-right: 0;">
    ...
    <div ref="gantt" style="height: 600px;margin-top: 0;"></div>
  </div>
</template>
...
<span style="position: absolute;left: 100%;margin-left: 5px;">${task.progress}%</span>
    ...
    updateTasks() {
      ...
      this.tasks.forEach(e => {
        e.dependencies.forEach(f => {
          links.push({ id: _.uniqueId(), source: f, target: e.id, type: e.linkType })
        })
      })
      gantt.config.start_date = moment(_.min(this.tasks.map(n => _.min([n.start_date, n.actual_start])))).subtract(
        1,
        'days'
      )
      gantt.config.end_date = moment(_.max(this.tasks.map(n => _.max([n.end_date, n.actual_end])))).add(1, 'days')
      gantt.parse({
        data: this.tasks,
        links
      })
    ...
    getBaseLineList() {
      this.axios.schedule.get('schedule/plan/listBase/' + localStorage.getItem('projectId')).then(res => {
