 ...
 * @Version: 
 * @Autor: 申豪强
 * @Date: 2020-02-21 09:19:08
 * @LastEditTime: 2020-04-23 09:42:54
 * 修改人：申豪强，修改内容：文件管理-按照起止时间查询-重置-再选择时间时出错；时间：2020-03-16 ；
 -->
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item style="text-align:right" label-width="10px">
            ...
            <el-button v-if="$access('ShareCreate')" type="primary" @click="creatFolder">新建文件夹</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
import previewVue from '../../breakingNews/preview.vue'
    ...
    return {
      ...
      tableColumn: [
        { type: 'selection', width: 50 },
        {
          ...
          render: (h, params) => {
            if (params.row.type === 1) {
              ...
              /*
               * @任务编号: 2748 | 普通文件的在线预览，其他模块同理，统一在此声明
               * @开发人员:申豪强
               * @日期:2020-03-30
               * @代码类型：新功能
               */
        ...
        { prop: 'createTime', label: '上传时间' },
        {
          ...
          label: '操作',
          render: (h, { row }) => {
            let downLoad = this.$access('ShareDownload')
              ? h(
                  'el-button',
                  {
                    props: {
              ...
              : null
            let deleteBtn = this.$access('ShareDelete')
              ? h(
                  'el-button',
                  {
                    props: {
              ...
              : null
            let goFloder = this.$access('ShareGoFloder')
              ? h(
                  'el-button',
                  {
                    props: {
              ...
              : null
            // 轻量化
            let lightWeight = this.$access('ShareLightWeight')
              ? h(
                  'el-button',
                  {
                    props: {
